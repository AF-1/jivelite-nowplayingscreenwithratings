...
function _getRating(self)
	...
	log:debug('coloredTrackTitleKeyword = '..tostring(coloredTrackTitleKeyword))

	if (thisTrackID and (tonumber(thisTrackID) > 0) and server) then
		server:userRequest(function(chunk,err)
				if err then
					log:info(err)
				else
					local currentRatingInfo, currentCommentInfo
					local songinfoLoopData = chunk.data.songinfo_loop
					log:debug("----------- songinfo_loop ----------\n"..dump(songinfoLoopData).."\n-----------\n\n")
					if (songinfoLoopData and type(songinfoLoopData) == 'table') then
						for k,v in pairs(songinfoLoopData) do
							if type(k) == 'number' then
								if songinfoLoopData[k].rating then currentRatingInfo = songinfoLoopData[k].rating end
								if songinfoLoopData[k].comment then currentCommentInfo = songinfoLoopData[k].comment end
							end
						end
					end
					...
					if (currentCommentInfo and currentCommentInfo ~= "" and coloredTrackTitleKeyword ~= nil and coloredTrackTitleKeyword ~= "" and currentCommentInfo:find(coloredTrackTitleKeyword)) then
